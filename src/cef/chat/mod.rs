...
impl Chat {
    ...
    pub fn initialize(&mut self) {
        ...
        self.chat_received.on(
            |ChatReceivedEvent {
                ...
                Self::handle_chat_received(message.to_string(), *message_type);
            },
        );
    }
    ...
    pub fn handle_chat_received(message: String, message_type: MsgType) {
        ...
        if let Some((_name, message)) = Self::get_player_name_and_message(message) {
            ...
            if split
                .get(0)
                .map(|first| Self::remove_color(first).trim() == "cef")
                .unwrap_or(false)
            {
                split.remove(0);

                let _ignore_error = command_callback(split);
            }
        }
    }
