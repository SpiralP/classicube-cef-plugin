...
impl PlayerTrait for HlsPlayer {
    ...
    fn on_create(&mut self) -> String {
        debug!("HlsPlayer on_create {}", self.url);

        format!(
            "data:text/html;base64,{}",
            base64::encode(
                PAGE_HTML
                    .replace("HLS_URL", &self.url)
    ...
    fn on_title_change(&mut self, _entity_id: usize, _browser: &RustRefBrowser, title: String) {
        if self.last_title == title || title == "HLS Stream Loading" {
            return;
        }
    ...
    pub fn from_url(url: &Url) -> Result<Self> {
        if url.scheme() != "http" && url.scheme() != "https" {
            ...
            match ext {
                ...
                _ => Err("url didn't end with a HLS .m3u8 file extension".into()),
            }
        }
    }
}
